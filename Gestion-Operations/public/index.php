...
<body>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="mb-3">
                        <label for="devise_code_modal" class="form-label">Devise</label>
                        <select name="devise_code_modal" id="devise_code_modal" class="form-select" required>
                            <option value="">-- Sélectionnez --</option>
                            <?php foreach ($devises as $devise): ?>
                                <option value="<?= htmlspecialchars($devise['code']) ?>"> <?= htmlspecialchars($devise['code']) ?></option>
                            <?php endforeach; ?>
                    ...
                    <button class="btn btn-success mt-3" onclick="generateBorderauModal()">Générer le Bordereau</button>
                    <button class="btn btn-primary mt-3" onclick="generateCC139Modal()">Générer le cc139</button>
                    <table class="table table-bordered" id="operationsTable">
                        <thead>
                            <tr>
                                <th><input type="checkbox" onclick="toggleSelectAllModal(this)"></th>
                                <th>Nature</th>
    ...
    <script>
                ...
                fetch('operations.php', {
                    ...
                    .then(result => {
                        if (result.status === 'success') {
                            ...
                            const newRow = `
                                <tr>
                                    <td><input type="checkbox" class="operation-checkbox-modal" value="${result.id}"></td>
                                    <td>${document.getElementById('nature').selectedOptions[0].text}</td>
                                    <td>${donneur}</td>
                                    <td>${objet}</td>
                                    <td>${devise_code}</td>
        ...
        function toggleSelectAllModal(source) {
            const checkboxes = document.querySelectorAll('.operation-checkbox-modal');
            checkboxes.forEach(checkbox => checkbox.checked = source.checked);
        }

        function generateBorderauModal() {
            const selectedOperations = Array.from(document.querySelectorAll('.operation-checkbox-modal:checked'))
            ...
            const deviseCode = document.getElementById('devise_code_modal').value;
